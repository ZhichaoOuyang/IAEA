...
def main(unused_argv):
  ...
  hparam_list = ['model', 'mode', 'eval_method', 'selector_loss_wt', 'inconsistent_loss', 'inconsistent_topk', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 'max_grad_norm', 'hidden_dim_selector', 'hidden_dim_rewriter','emb_dim',
                 'batch_size', 'max_art_len', 'max_sent_len', 'max_dec_steps', 'max_enc_steps', 'coverage', 'cov_loss_wt', 'eval_gt_rouge', 'decode_method', 'lr', 'gamma', 'eta', 'fixed_eta',
                 'reward_function', 'intradecoder', 'use_temporal_attention', 'rl_training', 'matrix_attention', 'pointer_gen']
  hps_dict = {}
  ...
  if FLAGS.model == 'selector':   # 选择句子
    ...
    if hps.mode == 'train':   # 训练阶段
      ...
      run_end2end.setup_training(end2end_model, batcher)   # 训练设置，传入模型器和批次的数据
    elif hps.mode == 'eval':
