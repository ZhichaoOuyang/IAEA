...
import sys
sys.path.append(r'/workspace/data/chao/IAEA/IAEA-Model/abstractor')     # change your own file path
...
class Rewriter(object):
  ...
  def _add_decoder(self, emb_dec_inputs, embedding):
    ...
    prev_coverage = self.prev_coverage if (hps.mode=="evalall" and hps.coverage) else None # In decode mode, we run attention_decoder one step at a time and so need to pass in the previous step's coverage vector each time
    prev_decoder_outputs = self.prev_decoder_outputs if (hps.intradecoder and hps.mode=="evalall") else tf.stack([],axis=0)
    prev_encoder_es = self.prev_encoder_es if (hps.use_temporal_attention and hps.mode=="evalall") else tf.stack([],axis=0)
    ...
    return attention_decoder(hps,
      ...
      initial_state_attention=(hps.mode=="evalall"),
  ...
  def _add_seq2seq(self, selector_probs=None):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      with tf.variable_scope('embedding'):
        if FLAGS.embedding:
          embedding = tf.Variable(self.embedding_place, trainable=False)
        else:
    ...
    if hps.mode == "train" or hps.mode == "eval":
  ...
  def _add_shared_loss_op(self):
    ...
    with tf.variable_scope('shared_loss'):
      ...
      self.variable_summaries('p_gen', self.p_gen_avg)
      ...
      if self._hps.rl_training:
        ...
        for dec_step, dist in enumerate(self.final_dists):
          ...
          for _k, targets in enumerate(tf.unstack(_targets,axis=1)): # list of k samples of size (batch_size)
            # print(batch_nums)
            # print(tf.to_int32(targets))
            indices = tf.stack((batch_nums, tf.to_int32(targets)), axis=1) # shape (batch_size, 2)
            gold_probs = tf.gather_nd(dist, indices) # shape (batch_size). prob of correct words on this step
            losses = -tf.log(gold_probs)
            loss_per_step.append(losses)
      ...
      if self._hps.coverage:
        ...
        if self._hps.rl_training or self._hps.ac_training:
          with tf.variable_scope('reinforce_loss'):
            print("_reinforce_cov_total_loss you zai#####################################################")
  ...
  def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage, prev_decoder_outputs, prev_encoder_es, selector_probs=None):
    ...
    if self._hps.model == 'end2end':
      ...
      to_return['attn_dists_norescale'] = self.temporal_es
    ...
    return results['ids'], results['probs'], new_states, temporal_e, attn_dists, final_dists, p_gens, new_coverage, output, temporal_e
