...
class SentenceSelector(object):
  ...
  def _add_sent_selector(self):     # Add the whole sequence-to-sequence model to the graph.
    ...
    with tf.variable_scope('SentSelector'):    # 设置指定作用域来共享变量
      ...
      #(3200,50,200*2)
      sent_enc_outputs,sent_outputs_state = self._add_encoder(sent_enc_inputs, sent_lens, name='sent_encoder') # (batch_size*max_art_len, max_sent_len, hidden_dim*2)  # 每个句子在句子级别的表示。 一个句子由首尾拼接起来的hidden来表示
      ...
      sent_feats = tf.contrib.layers.fully_connected(art_enc_outputs, hps.hidden_dim_selector, activation_fn=tf.tanh) # (batch_size, max_art_len, hidden_dim)   # 句子的embedding
      ...
      weight_Dense_1 = np.load("data/relevant_weight.npy")
      bias_Dense_1 = np.load("data/relevant_bias.npy")
      print((weight_Dense_1.shape, " ", bias_Dense_1.shape))


      weight_Dense_2 = np.load("data/inconsistent_weight.npy")
      bias_Dense_2 = np.load("data/inconsistent_bias.npy")
      ...
      art_feats = tf.reduce_sum(art_enc_outputs * art_padding_mask, axis=1) / tf.reduce_sum(art_padding_mask, axis=1) # (batch_size, hidden_dim * 2)    句子相加得到一个简单的文章表示
      ...
      def inconsistent(old, new):  # 不一致概率大于0.5的
          ...
          with tf.variable_scope("Sequencer", reuse=True):  # 调用之前的变量
              ...
              novelty_feats = tf.reduce_sum(tf.matmul(new_sent[:, 0, :], w_novelty) * tf.tanh(s), 1,
                                            keep_dims=True)  # (, 1)
              logist = content_feats + salience_feats - novelty_feats + bias
          ...
          mid = laterNewTweet_logist[:, new - 10] * result2[:, 0] * p[:, 0]
      ...
      for i in range(10, hps.max_art_len):
          for j in range(10):
              ...
              result1 = (tf.nn.sigmoid(tf.matmul(subed, weight_Dense_1) + bias_Dense_1, name="irrelevant"))
              result2 = (tf.nn.sigmoid(tf.matmul(subed, weight_Dense_2) + bias_Dense_2, name="inconsistent"))
              ...
              firstTenOldSummary = tf.where( (tf.greater_equal(result1[:, 0], 0.5)) & (tf.greater_equal(result2[:, 0], 0.5)) & (
              ...
              laterNewTweet = tf.where((tf.greater_equal(result1[:, 0], 0.5)) & (tf.greater_equal(result2[:, 0], 0.5)) & (
                  tf.not_equal(tf.reduce_sum(aeqb_int), tf.reduce_sum(tf.ones_like(aeqb_int)))),
                                       dict_list_inconsistent['update_new_tweet'],
                                       dict_list_consistent['update_new_tweet'])
              firstTenOldSummary_logist = tf.where((tf.greater_equal(result1[:, 0], 0.5)) & (tf.greater_equal(result2[:, 0], 0.5)) & (
                  tf.not_equal(tf.reduce_sum(aeqb_int), tf.reduce_sum(tf.ones_like(aeqb_int)))),
                                                   dict_list_inconsistent['update_old_summary_logist'],
                                                   dict_list_consistent['update_old_summary_logist'])
              laterNewTweet_logist = tf.where((tf.greater_equal(result1[:, 0], 0.5)) & (tf.greater_equal(result2[:, 0], 0.5)) & (
                  tf.not_equal(tf.reduce_sum(aeqb_int), tf.reduce_sum(tf.ones_like(aeqb_int)))),
                                              dict_list_inconsistent['update_new_tweet_logist'],
                                              dict_list_consistent['update_new_tweet_logist'])
              final_score = tf.where((tf.greater_equal(result1[:, 0], 0.5)) & (tf.greater_equal(result2[:, 0], 0.5)) & (
                  tf.not_equal(tf.reduce_sum(aeqb_int), tf.reduce_sum(tf.ones_like(aeqb_int)))),
                                     dict_list_inconsistent['update_final_score'],
                                     dict_list_consistent['update_final_score'])
              summary_index = tf.where((tf.greater_equal(result1[:, 0], 0.5)) & (tf.greater_equal(result2[:, 0], 0.5)) & (
                  tf.not_equal(tf.reduce_sum(aeqb_int), tf.reduce_sum(tf.ones_like(aeqb_int)))),
