...
class SentenceSelector(object):
  ...
  def tweet_sequencer(self, sent_feats, art_feats):
    ...
    hidden_dim = self._hps.hidden_dim_selector * 2
  ...
  def _add_sent_selector(self):     # Add the whole sequence-to-sequence model to the graph.
    ...
    with tf.variable_scope('SentSelector'):    # 设置指定作用域来共享变量
      ...
      sent_enc_outputs, sent_outputs_state = self._add_encoder(sent_enc_outputs, sent_lens, name='second_sent_encoder') # 第一个(batch_size*max_art_len, max_sent_len, hidden_dim*2
      ...
      art_enc_inputs = tf.reduce_max(sent_enc_outputs * sent_padding_mask, axis=1)
      print("max_pooling," , art_enc_inputs)  #  # (batch_size*max_art_len, hidden_dim*2) （500,200）
      ...
      sent_feats = tf.contrib.layers.fully_connected(art_enc_inputs, hps.hidden_dim_selector*2, activation_fn=tf.tanh) # (batch_size, max_art_len, hidden_dim)   # 句子的embedding
      ...
      art_feats = tf.contrib.layers.fully_connected(art_feats, hps.hidden_dim_selector*2, activation_fn=tf.tanh) # (batch_size, hidden_dim)   # 文档的embedding 在用一个全连接层得到最后 一个文章的表示
      ...
      zeor_tensor = tf.zeros([hps.hidden_dim_selector*2], dtype=tf.float32)
      ...
      def inconsistent(old, new):  # 不一致概率大于0.5的
          ...
          s = tf.zeros([hps.batch_size, hps.hidden_dim_selector*2])
          ...
          with tf.variable_scope("Sequencer", reuse=True):  # 调用之前的变量
              ...
              new_sent = tf.reshape(laterNewTweet[:, new - 10, :], [hps.batch_size, 1, hps.hidden_dim_selector*2])
          ...
          mid = tf.reshape(laterNewTweet[:, new - 10, :], [hps.batch_size, 1, hps.hidden_dim_selector*2])  # 把新的那个推文加进来
          ...
          mid = tf.constant(0, shape=[hps.batch_size, 1, hps.hidden_dim_selector*2], dtype=tf.float32)
