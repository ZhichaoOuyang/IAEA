...
import abstractor.run_rewriter as run_rewriter
...
tf.app.flags.DEFINE_integer('start_eval_rouge', 300, 'for rouge mode, start evaluating rouge scores after this iteration')
...
tf.app.flags.DEFINE_integer('emb_dim', 100, 'dimension of word embeddings')   # Glove 100 dimension
...
# Pointer-generator with Self-Critic policy gradient: https://arxiv.org/pdf/1705.04304.pdf   abstractor
...
def main(unused_argv):
  ...
  FLAGS.log_root = os.path.join(FLAGS.log_root, FLAGS.model, FLAGS.exp_name)   # abstractor save path
  ...
  if FLAGS.model == 'selector':   # extractor
    ...
    if hps.mode == 'train':
      ...
      end2end_model = SelectorRewriter(hps, select_model, rewrite_model)     # end2end init
      run_end2end.setup_training(end2end_model, batcher, vocab.getWordEmbedding())   # train setting
    elif hps.mode == 'eval':
