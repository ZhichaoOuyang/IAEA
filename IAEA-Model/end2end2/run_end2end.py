...
def setup_training(model, batcher, word_vector):     # end-end 训练的设置，model是两器模型，batcher是具体的数据传入
  """Does setup before starting training (run_training)"""
  train_dir = os.path.join(FLAGS.log_root, "train")    # 实验结果存放的位置
  ...
  if FLAGS.pretrained_selector_path and FLAGS.pretrained_rewriter_path:    # 如果有预训练的模型
    params = tf.global_variables()    # 得到全部的变量
    ...
    selector_saver = tf.train.Saver(selector_vars)   # 抽取器的保存器
    rewriter_saver = tf.train.Saver(rewriter_vars)   # 生成器的保存器
    local_init_op = tf.variables_initializer(uninitialized_vars)   # 不是抽取器也不是生成器的变量
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer, word_vector, \
  ...
  with sess_context_manager as sess:
    if FLAGS.pretrained_selector_path:    # 加载预训练模型
