...
class SelectorRewriter(object):
  ...
  def build_graph(self):   # end-end 图结构
    """Add the placeholders, model, global step, train_op and summaries to the graph"""
    tf.logging.info('Building graph...')
    t0 = time.time()
    self._selector._add_placeholders()   # 抽取器的占位符
    self._rewriter._add_placeholders()   # 生成器的占位符
    # with tf.device("/gpu:3"):
    self._selector._add_sent_selector()     # 抽取器的选择过程，只到loss的计算函数
    self._rewriter._add_seq2seq(selector_probs=self._selector.probs)   # 通过probs调整单词级别的\alpha attention   生成的过程，也只到loss的计算函数
